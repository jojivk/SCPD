...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
           nsent =[]
           for word in sent:
              nword=[self.start_of_word]
              for ch in word :
                 nword.append(self.char2id[ch])
              nword.append(self.end_of_word)
              nsent.append(nword)
           word_ids.append(nsent)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        #print("JMJ : Batch Size Sents:", len(sents))
        w2ind = self.words2charindices(sents)
        padded_sents = pad_sents_char(w2ind, self['<pad>'])
