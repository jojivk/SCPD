...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded = self.vocab.src.to_input_tensor(source, device=self.device).contiguous()
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device).contiguous()
        ...
        source_padded_chars  = source_padded_chars.contiguous()
