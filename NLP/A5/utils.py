...
def show_sents(padded_sentences):
...
def pad_sents_char(sents, char_pad_token):
    ...
    sents_padded =[]
    pad = [char_pad_token]*max_word_length
    ...
    for sent in sents:
       ...
       for word in sent :
           nword= pad.copy()
           if len(word) <=21 :
             nword[0:len(word)] = word.copy()
           else :
             nword = word[:21].copy() 
           nsent.append(nword)
           #print(nword)
       si = len(nsent)
       for i in range(si, max_len):
           nsent.append(pad.copy())
       sents_padded.append(nsent)

    #print("JMJ: Max Sent len:", len(sents_padded[1]))
    #show_sents(sents_padded)
    return sents_padded
    ### END YOUR CODE
